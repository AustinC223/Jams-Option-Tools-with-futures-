...
/* ---------- Slider ---------- */
div[data-testid="stSlider"] [data-baseweb="slider"]{ padding-top:6px !important; }
div[data-testid="stSlider"] [data-baseweb="slider"] *{ color:#E6E6E6 !important; }
...
def bs_delta(S, K, T, r, q, sigma, opt_type):
    ...
    if opt_type == "call": return math.exp(-q*T) * norm_cdf(d1)
...
def charm_fd(S, K, T, r, q, sigma, opt_type, dt_days=1):
    ...
    if d_now is None or d_next is None: return None
    return (d_next - d_now) / dt

def prob_finish_beyond(S0, L, T, r, q, sigma, direction: str):
    if T <= 0 or sigma <= 0 or S0 <= 0 or L <= 0: return None
    mu = (r - q - 0.5*sigma*sigma)*T
    denom = sigma * math.sqrt(T)
    z = (math.log(L / S0) - mu) / denom
    if direction == "above": return 1.0 - norm_cdf(z)
    return norm_cdf(z)

def prob_touch_barrier(S0, B, T, r, q, sigma, barrier_type: str):
    if T <= 0 or sigma <= 0 or S0 <= 0 or B <= 0: return None
    if barrier_type == "up" and B <= S0: return 1.0
    if barrier_type == "down" and B >= S0: return 1.0
...
def bucket_label(dte: int) -> str:
    if dte <= SHORT_DAYS: return "Short (≤14D)"
    if dte <= MID_DAYS: return "Mid (15–180D)"
    return "Long (≥181D)"
...
def pick_atm_iv(chain: pd.DataFrame, spot: float, target_days: int):
    ...
    if not ivs: return None, exp, atm_k
...
def db_init():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS snapshots (
            snapshot_id TEXT PRIMARY KEY, ts_utc TEXT, ticker TEXT, spot REAL, spot_ts TEXT
        )
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS chain_rows (
            snapshot_id TEXT, expiry TEXT, dte INTEGER, option_type TEXT, strike REAL, volume INTEGER, open_interest INTEGER
        )
    """)
    con.commit()
    con.close()
...
def snapshot_store(ticker: str, spot: float, spot_ts: str, chain: pd.DataFrame) -> str:
    ...
    con = sqlite3.connect(DB_FILE)
    cur = con.cursor()
    cur.execute("INSERT INTO snapshots VALUES (?,?,?,?,?)", (sid, datetime.utcnow().isoformat(), ticker, float(spot), str(spot_ts)))
    rows = []
...
def get_last_two_snapshots(ticker: str):
    ...
    con = sqlite3.connect(DB_FILE)
    cur = con.cursor()
    cur.execute("SELECT snapshot_id, ts_utc, spot FROM snapshots WHERE ticker=? ORDER BY ts_utc DESC LIMIT 2", (ticker,))
    rows = cur.fetchall()
    con.close()
    return rows
...
def screener_watchlist(tickers, price_move_max_pct=1.0, oi_jump_min=5000, vol_jump_min=5000):
    ...
    for tkr in tickers:
        ...
        if abs(chg) > price_move_max_pct: continue

        cur.execute("SELECT option_type, SUM(open_interest), SUM(volume) FROM chain_rows WHERE snapshot_id=? GROUP BY option_type", (s0,))
        new = {r[0]: (r[1] or 0, r[2] or 0) for r in cur.fetchall()}
        cur.execute("SELECT option_type, SUM(open_interest), SUM(volume) FROM chain_rows WHERE snapshot_id=? GROUP BY option_type", (s1,))
        ...
        for side in ["call", "put"]:
            ...
            if (oi_jump >= oi_jump_min) or (vol_jump >= vol_jump_min):
                out.append({
                    "ticker": tkr, "side": side, "spot_change_pct": chg,
                    "oi_jump_total": oi_jump, "vol_jump_total": vol_jump,
                    "new_snapshot_utc": ts0, "old_snapshot_utc": ts1,
                })
    con.close()
...
def style_fig(fig, title: str):
    fig.update_layout(
        template="plotly_dark", height=520, title=title,
        paper_bgcolor="#000000", plot_bgcolor="#000000",
        font=dict(color="#E6E6E6", size=13), margin=dict(l=10, r=10, t=70, b=10),
...
def add_spot_line(fig, spot: float, ymax: float):
    ...
    fig.add_annotation(
        x=spot, y=ymax, xref="x", yref="y", text=f"Spot {spot:,.2f}",
        showarrow=True, arrowhead=2, ax=30, ay=-40, font=dict(color="#FF991C", size=13),
        bgcolor="rgba(0,0,0,0.65)", bordercolor="rgba(255,153,28,0.8)", borderwidth=1
    )
    return fig
...
def strike_3bar_frame(df: pd.DataFrame, metric: str) -> pd.DataFrame:
    ...
    for c in ["Call", "Put", "Total"]: out[c] = out[c].round(0).astype(int)
    return out
...
# UI CONTROLS
# =========================
st.sidebar.markdown("## Options Controls")
ticker = st.sidebar.text_input("Options Ticker", value="SPY").upper().strip()
...
refresh = c1.button("Refresh Options Data", disabled=not refresh_allowed())
...
with tabs[0]:
    ...
    d = chain.copy()
    if scope != "All": d = d[d["bucket"] == scope]
    if use_window == "Around spot":
        lo = spot * (1 - window_pct / 100); hi = spot * (1 + window_pct / 100)
...
with tabs[1]:
    ...
    if use_window2 == "Around spot":
        lo = spot * (1 - window_pct2 / 100); hi = spot * (1 + window_pct2 / 100)
        d = d[(d["strike"] >= lo) & (d["strike"] <= hi)]
...
with tabs[2]:
    ...
    def top_levels_for_bucket(bucket_name: str, days: int, n=3):
        ...
        if side3 in ("call", "put"): subset = subset[subset["option_type"] == side3]
        ...
        def add_probs(df, kind):
            if df.empty or not sigma:
                df["P_touch"], df["P_finish"] = np.nan, np.nan
                return df
            pt, pf = [], []
    ...
    for bucket_name, days in [("Short (≤14D)", SHORT_DAYS), ("Mid (15–180D)", MID_DAYS), ("Long (≥181D)", LONG_DAYS)]:
        ...
        cL, cR = st.columns(2)
        with cL: st.markdown("**Support**"); st.dataframe(sup, use_container_width=True, height=160)
        with cR: st.markdown("**Resistance**"); st.dataframe(res, use_container_width=True, height=160)
...
with tabs[3]:
    ...
    d = chain.copy()
    if scope4 != "All": d = d[d["bucket"] == scope4]
    if side4 in ("call", "put"): d = d[d["option_type"] == side4]
...
with tabs[4]:
    ...
    if d.empty:
        ...
        figH = go.Figure(data=go.Heatmap(
            z=piv.values, x=piv.columns.astype(float), y=[str(x) for x in piv.index],
            colorbar=dict(title="IV"), colorscale="Viridis"
        ))
        figH.update_layout(
            template="plotly_dark", height=560, title="IV Heatmap: Expiry × Strike",
...
# =========================
# TAB 7: FUTURES MICROSTRUCTURE (NEW)
# =========================
with tabs[6]:
    ...
    if f_data.empty:
        st.warning("Failed to fetch futures data. Please check ticker symbols (e.g., ESM5=F, ESU5=F, ^GSPC) in the sidebar.")
    else:
        fig_f = make_subplots(
            rows=3, cols=1,
            shared_xaxes=True,
            ...
            subplot_titles=("1. Futures vs Spot Basis", "2. Calendar Spread (Term Structure)", "3. Volume Rollover (Near vs Far)"),
            specs=[[{"secondary_y": True}], [{}], [{}]] # 第一張圖啟用雙 Y 軸給基差使用
        )

        # --- Row 1: 價格與基差 ---
        fig_f.add_trace(go.Scatter(x=f_data.index, y=f_data['Near_Close'], name="Near Futures", line=dict(color="#00B3FF", width=2)), row=1, col=1, secondary_y=False)
        fig_f.add_trace(go.Scatter(x=f_data.index, y=f_data['Spot_Close'], name="Spot Index", line=dict(color="#E6E6E6", width=2, dash="dot")), row=1, col=1, secondary_y=False)

        # 綠色正基差，紅色負基差
        basis_colors = ['rgba(0,255,65,0.4)' if val >= 0 else 'rgba(255,45,170,0.6)' for val in f_data['Basis']]
        fig_f.add_trace(go.Bar(x=f_data.index, y=f_data['Basis'], name="Basis Pts", marker_color=basis_colors), row=1, col=1, secondary_y=True)

        # --- Row 2: 跨期價差 (Calendar Spread) ---
        # 橘色 Contango，紅色 Backwardation
        spread_colors = ['#FF991C' if val >= 0 else '#FF2DAA' for val in f_data['Calendar_Spread']]
        fig_f.add_trace(go.Bar(x=f_data.index, y=f_data['Calendar_Spread'], name="Calendar Spread", marker_color=spread_colors), row=2, col=1)

        # --- Row 3: 成交量交叉 (Volume Rollover) ---
        fig_f.add_trace(go.Scatter(x=f_data.index, y=f_data['Near_Volume'], name="Near Vol", line=dict(color="#00B3FF", width=2), fill='tozeroy', fillcolor='rgba(0,179,255,0.1)'), row=3, col=1)
        fig_f.add_trace(go.Scatter(x=f_data.index, y=f_data['Far_Volume'], name="Far Vol", line=dict(color="#FF991C", width=2), fill='tozeroy', fillcolor='rgba(255,153,28,0.2)'), row=3, col=1)
        ...
        fig_f.update_xaxes(gridcolor="rgba(255,255,255,0.08)")
        fig_f.update_yaxes(gridcolor="rgba(255,255,255,0.08)", secondary_y=False)
        fig_f.update_yaxes(showgrid=False, secondary_y=True) # 隱藏副 Y 軸的網格線避免凌亂

        st.plotly_chart(fig_f, use_container_width=True)

        st.info("""
        **Microstructure 判讀指南：**
        * **Row 1 (Basis)：** 藍線為期貨，白虛線為現貨。背景柱狀圖若出現 **深紅色深谷 (大幅負基差)**，代表市場存在強烈避險情緒或現貨拋售預期。
        * **Row 2 (Term Structure)：** 正常為橘色柱子 (Contango)。若柱子轉為 **紅色向下 (Backwardation)**，代表近月合約遭瘋狂搶買，為極端恐慌或空頭急遽回補的特徵。
        * **Row 3 (Volume Rollover)：** 當橘線 (遠月) 由下往上穿越藍線 (近月) 形成黃金交叉時，即為確認主力資金完成轉倉的關鍵日。
        """)
